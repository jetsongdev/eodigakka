...
- [ ] **강남권 최근 거래 티커** — 강북 14구 티커 PR 안정화 후 후속. `TARGET_GU = SEOUL_25` 확장 이후 강남권(강남·서초·송파·강동 우선) 최근 RTMS 매매·순수전세 50건을 별도 티커/지역 토글로 노출 검토. 단정문·추천 없이 사실 표시만, 기존 `/api/recent` 캐시·silent degrade 패턴 재사용.
