...
| Phase | 범위 | 기간 |
|---|---|---|
| 0 | real-estate-mcp 검증(MCP stdio 연결 확인) + 강북14구 1개월치 손으로 받아 감 잡기. MCP 실패 시 PublicDataReader CLI fallback (ADR-004) | 0.5일 |
| 1 | 강북 14구 매매 + 전세 ETL → PostGIS, 색칠지도 1장 | 3일 |
| 2 | 서울 25구 전세, 전세가율 가드, 사이드패널 | 2일 |
| 3 | (선택) 외곽 매매 추가, 정책대출 옵션 |  |
| 4 | (선택) 자체 MCP tool: `find_affordable_dongs` |  |
